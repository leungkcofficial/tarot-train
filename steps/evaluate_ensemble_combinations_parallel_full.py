"""
Optimized step to evaluate ALL ensemble combinations using parallel processing with checkpointing.
"""
...
@step
def evaluate_ensemble_combinations_parallel_full(
    ...
    """
    Evaluate ALL ensemble combinations using parallel processing with checkpointing.
    ...
    """
    print("\nGenerating ALL ensemble combinations...")
    ...
    print("Generating combination list...")
    sys.stdout.flush()
    ...
    for length in range(2, 25):  # From 2 to 24 models
        ...
        for combo in combinations(range(24), length):
            ...
            combo_count += 1
        print(f"  - {length} models: {combo_count:,} combinations")
        sys.stdout.flush()
    ...
    print(f"\nProcessing combinations in batches of {batch_size:,}")
    print("=" * 80)
    sys.stdout.flush()
    
    for batch_start in range(start_idx, total_combinations, batch_size):
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] Processing batch {batch_start//batch_size + 1} ({len(batch_combos):,} combinations)...")
        sys.stdout.flush()
        ...
        print(f"\n[{datetime.now().strftime('%H:%M:%S')}] Batch {batch_start//batch_size + 1} completed")
    ...
    # Save summary statistics - including event-specific IBS
