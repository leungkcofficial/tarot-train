"""
Multi-Model Deployment Pipeline for CKD Risk Prediction (Version 2)

This module implements ensemble predictions from multiple pre-trained models.
It can either load pre-trained models or retrain them with their specific configurations.
"""
...
def create_model_specific_config(model_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    config = {
        ...
        'balance': {
            ...
            'method': 'near_miss' if 'NearMiss' in balancing_method else ('knn' if balancing_method == 'KNN' else 'random_under_sampler'),
...
def create_and_train_model(
    ...
    hp_config = create_model_specific_config(model_config)
    ...
    if not retrain:
        ...
        # Retrain the model with specific configuration
        print(f"\n=== Retraining Model {model_config['model_no']} ===")
        print(f"Using balancing: {model_config['balancing_method']}")
        print(f"Optimization target: {model_config['optimization_target']}")
        
        # Apply balancing if needed
        train_data_balanced = train_data.copy()
        balance_cfg = hp_config['balance']
        
        if balance_cfg['enable']:
            print(f"Applying {balance_cfg['method']} balancing...")
            endpoint = hp_config.get('target_endpoint')
            
            train_data_balanced = balance_dataframe(
                train_data_balanced,
                duration_col="duration",
                event_col="endpoint",
                method=balance_cfg['method'],
                sampling_strategy=balance_cfg['sampling_strategy'],
                model_type=model_type,
            ...
            print(f"Balanced data shape: {train_data_balanced.shape}")
        
        # Prepare training data
        endpoint = hp_config.get('target_endpoint')
        train_dataset = prepare_survival_dataset(
            train_data_balanced,
            feature_cols=feature_cols,
            target_endpoint=endpoint
        )
        
        X_train, durations_train, events_train = train_dataset
        
        # Convert to tensors
        x_train = torch.tensor(X_train, dtype=torch.float32).to(device)
        durations_train = torch.tensor(durations_train, dtype=torch.float32)
        events_train = torch.tensor(events_train, dtype=torch.float32)
        
        # For DeepHit, discretize the data
        if model_type.lower() == 'deephit':
            from pycox.preprocessing.label_transforms import LabTransDiscreteTime
            labtrans = LabTransDiscreteTime(model.duration_index)
            y_train = labtrans.fit_transform(durations_train.numpy(), events_train.numpy())
        else:
            y_train = (durations_train, events_train)
        
        # Train the model
        callbacks = [EarlyStopping(patience=hp_config['optimization']['patience'])]
        batch_size = model_details.get('batch_size', 64)
        epochs = model_details.get('epochs', 100)
        ...
        # Compute baseline hazards for CoxPH
        if model_type.lower() == "deepsurv":
        ...
        new_model_path = f"models/retrained/model{model_config['model_no']}_retrained_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pt"
        torch.save(model.net.state_dict(), new_model_path)
        print(f"Saved retrained model to {new_model_path}")
...
def multi_model_deploy_pipeline():
    ...
    ensemble_results = deploy_multiple_models_v2(
        ...
        retrain_models=False  # Set to True to retrain models with their specific configs
    )
...
def deploy_multiple_models_v2(
    ...
    Args:
        ...
        batch_size: Batch size for processing
        retrain_models: Whether to retrain models with their specific configurations
        
    Returns:
        Dictionary with deployment results
    """
    print("\n=== Multi-Model Deployment Pipeline V2 ===\n")
    ...
    for config in model_configs:
        ...
        try:
            ...
            model = create_and_train_model(
                model_config=config,
                train_data=train_df_preprocessed,
                feature_cols=feature_cols,
                device=device,
                retrain=retrain_models
            )
            
            # For DeepSurv, compute baseline hazards if not retrained
            if config['algorithm'] == 'DeepSurv' and not retrain_models:
