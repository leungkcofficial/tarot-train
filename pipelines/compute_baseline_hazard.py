"""
Compute Baseline Hazards Pipeline for DeepSurv Models

This pipeline computes and saves baseline hazards for all DeepSurv models
in the final deployment directory. It reuses the data processing steps from
the ensemble deployment pipeline and adds a new step to compute baseline hazards.
"""
...
def compute_baseline_hazards_pipeline():
    """
    Pipeline for computing baseline hazards for all DeepSurv models.
    
    This pipeline:
    1. Processes data using the same steps as ensemble_deploy_pipeline
    2. Loads all DeepSurv models from results/final_deploy/models
    3. Computes baseline hazards for each model using training data
    4. Saves baseline hazards as .pkl files
    
    Returns:
        Summary of baseline hazard computation results
    """
    ...
    # Import new step for computing baseline hazards
    from steps.compute_all_baseline_hazards import compute_all_baseline_hazards
    ...
    # ==================== Baseline Hazard Computation ====================
    
    print("\n=== Computing Baseline Hazards for All DeepSurv Models ===")
    
    # Step 7: Compute baseline hazards for all DeepSurv models
    ...
    print("\n=== Baseline Hazard Computation Pipeline Complete ===")
    # Note: In ZenML, we can't directly access the step output here
    # The results will be available in the step artifacts after the pipeline runs
...
if __name__ == "__main__":
    # Run the pipeline
    pipeline_instance = compute_baseline_hazards_pipeline()
