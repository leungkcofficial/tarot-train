"""
PyTorch Model Manager for TAROT CKD Risk Prediction
Handles loading and inference with the actual trained PyTorch ensemble models
"""
...
class PyCoxModelWrapper:
    """Wrapper for PyTorch models to standardize interface"""
    ...
    def load(self) -> bool:
        """Load PyTorch model and baseline hazards"""
        try:
            # Load PyTorch model
            self.model = torch.load(self.model_path, map_location=self.device)
            self.model.eval()
            
            # Load baseline hazards if available
    ...
    def _risk_scores_to_probabilities(self, risk_scores: np.ndarray, 
                                    time_horizons: List[int]) -> np.ndarray:
        """Convert risk scores to approximate probabilities"""
        # Simplified conversion - in practice you'd use proper calibration
        # Sigmoid transformation to get probabilities
        probs = 1 / (1 + np.exp(-risk_scores.reshape(-1)))
        
        # Scale by time horizon (higher risk at longer horizons)
        time_scaled_probs = []
        for horizon in time_horizons:
            scaled_prob = 1 - (1 - probs[0]) ** (horizon / 2.0)
            time_scaled_probs.append(min(scaled_prob, 0.95))  # Cap at 95%
        
        return 1 - np.array(time_scaled_probs)  # Return survival probabilities
...
class PyTorchModelManager:
    """Manages PyTorch ensemble models for TAROT CKD prediction"""
    ...
    async def load_models(self) -> None:
        ...
        try:
            ...
            logger.info(f"Successfully loaded {len(self.models)} models in {load_time:.2f}s")
    ...
    async def _load_ensemble_models(self) -> None:
        ...
        logger.info(f"Successfully loaded {loaded_count}/{len(results)} models")
    ...
    async def predict(self, patient_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Prediction complete: {len(model_predictions)} models, {inference_time*1000:.1f}ms")
    ...
    async def _preprocess_features(self, patient_data: Dict[str, Any]) -> np.ndarray:
        ...
        lab_values = {lab["parameter"]: lab["value"] for lab in patient_data.get("laboratory_values", [])}
        features.extend([
            lab_values.get("egfr", 30),
            lab_values.get("hemoglobin", 11),
            lab_values.get("phosphate", 3.5),
            lab_values.get("bicarbonate", 24),
            lab_values.get("uacr", 100)
        ])
        
        # Medical history
        medical_history_list = patient_data.get("medical_history") or []
        medical_history = {mh["condition"]: mh["diagnosed"] for mh in medical_history_list}
        features.extend([
            patient_data.get("charlson_score", 0),
            1 if medical_history.get("diabetes", False) else 0,
            1 if medical_history.get("hypertension", False) else 0,
            1 if medical_history.get("cardiovascular_disease", False) else 0
        ])
        ...
        if self.preprocessor is not None:
            loop = asyncio.get_event_loop()
            features_array = await loop.run_in_executor(
                self.executor, self.preprocessor.transform, features_array
            )
