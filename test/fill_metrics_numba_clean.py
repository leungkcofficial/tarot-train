"""
Clean version of Numba-optimized ensemble evaluation.
"""
...
def main():
    ...
    output_file = 'results/ensemble_checkpoints/evaluation_results_numba_clean.csv'
    batch_size = 100  # Smaller batch size for more frequent updates
    
    print("\n" + "="*80)
    print("NUMBA-OPTIMIZED ENSEMBLE EVALUATION (CLEAN VERSION)")
    print("="*80)
    ...
    for chunk_idx, chunk in enumerate(pd.read_csv(results_file, chunksize=batch_size)):
        ...
        batch_results = []
        for idx, row in chunk_to_process.iterrows():
            # Get model indices
    ...
    top_100.to_csv('results/ensemble_checkpoints/top_100_combinations_numba_clean.csv', index=False)
    print(f"\nSaved top 100 combinations to: results/ensemble_checkpoints/top_100_combinations_numba_clean.csv")
