...
def check_group_prediction_shapes():
    """Check the prediction shapes for each DeepSurv group"""
    ...
    print("=" * 80)
    print("Note: DeepSurv predictions shape = (time_points, n_samples)")
    print("=" * 80)
            ...
            if model_no in model_paths:
                ...
                try:
                    with h5py.File(model_info['temporal_test'], 'r') as f:
                        ...
                        print(f"      Temporal Test predictions shape: {pred_shape}")
                ...
                try:
                    with h5py.File(model_info['spatial_test'], 'r') as f:
                        pred_shape = f['predictions'].shape
                        print(f"      Spatial Test predictions shape: {pred_shape}")
                except Exception as e:
    ...
    print("=" * 80)
    print("Note: DeepHit predictions shape = (n_events, time_points, n_samples)")
    print("=" * 80)
    ...
    for model in deephit_models:
        ...
        try:
            with h5py.File(model['temporal_test_predictions_path'], 'r') as f:
                pred_shape = f['predictions'].shape
                cause1_shape = f['cause_1_predictions'].shape
                cause2_shape = f['cause_2_predictions'].shape
                print(f"  Temporal Test:")
                print(f"    - predictions shape: {pred_shape}")
                print(f"    - cause_1_predictions shape: {cause1_shape}")
                print(f"    - cause_2_predictions shape: {cause2_shape}")
        except Exception as e:
            print(f"  Error reading temporal test: {e}")
    ...
    print("=" * 80)
    print("\nDeepSurv models:")
    print("  - All models predict for 42,953 samples")
    print("  - Predictions have 1,825 time points")
    print("  - Shape: (1825, 42953)")
    print("\nDeepHit models:")
    print("  - All models predict for 42,953 samples")
    print("  - Predictions have 2 events and 5 time points")
    print("  - Combined predictions shape: (2, 5, 42953)")
    print("  - Individual cause predictions shape: (5, 42953)")
