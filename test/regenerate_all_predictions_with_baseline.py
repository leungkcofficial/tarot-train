...
def prepare_data_for_model(df, feature_cols, network_type, sequence_length=5, 
    ...
    if network_type == 'lstm':
        ...
        if target_endpoint is not None:
            # DeepSurv - single event
            durations = latest_obs[f'duration{target_endpoint}'].values.astype('float32')
            events = latest_obs[f'event{target_endpoint}'].values.astype('float32')
        else:
            # DeepHit - both events
...
def generate_predictions(model, model_type, X, durations, events):
...
def main():
    ...
    for i, model_info in enumerate(all_models):
        ...
        try:
            # Load the model
            model, network_type = load_model_for_prediction(model_info)
            if model is None:
            ...
            temporal_preds, temporal_c1, temporal_c2 = generate_predictions(
                model, model_type, temporal_X, temporal_durations, temporal_events
            )
            spatial_preds, spatial_c1, spatial_c2 = generate_predictions(
                model, model_type, spatial_X, spatial_durations, spatial_events
            )
    ...
    # Save summary
    summary_file = Path('results/test_predictions_regenerated/regeneration_summary.json')
