...
This version properly extracts paths from deployment details.
"""
...
for i, detail in enumerate(deployment_details):
    ...
    if i < 3:
        print(f"\nDetail {i}:")
        print(f"  Model config: {detail.get('model_config', {}).get('algorithm', 'Unknown')} - {detail.get('model_config', {}).get('structure', 'Unknown')}")
...
if not temporal_paths:
    ...
    # Sometimes the paths might be in a different location
...
if temporal_paths:
    ...
    print("Please check the structure of the cached artifacts.")